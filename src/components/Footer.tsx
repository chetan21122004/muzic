...
const centres = [
  {
    ...
    phone: "+91 9156 303 400",
  },
  {
    name: "Pimple Saudagar",
    address: "2nd Floor, Radhika Avenue, Opp. McDonalds, Pune 411027",
    phone: "+91 9156 303 400",
  },
  {
    name: "Hinjewadi",
    address: "Spirea, S.91/4, Bhumkar Das Gugre Rd, Wakad 411033",
    phone: "+91 9156 303 400",
  },
  {
    name: "Croydon, UK",
    address: "Croydon, London, United Kingdom",
    phone: "+44 7768 928 645",
  },
];

const Footer = () => {
  return (
    <footer className="relative bg-[#0a0a0f] text-gray-400 overflow-hidden border-t border-white/5">
      {/* Watermark */}
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full text-center pointer-events-none select-none z-0">
        <p className="text-[13vw] font-black text-white/[0.02] tracking-tighter whitespace-nowrap">
          LIVE MUSIC
        </p>
      </div>
      ...
      <div className="container mx-auto px-4 relative z-10">
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="space-y-2.5">
              <a href="tel:+919156303400" className="flex items-center gap-3 group w-fit">
                ...
                <span className="text-sm text-white/60 group-hover:text-white transition-colors font-medium">+91 9156 303 400</span>
              </a>
              <a href="tel:+447768928645" className="flex items-center gap-3 group w-fit">
                <div className="w-8 h-8 rounded-xl bg-primary/10 border border-primary/20 flex items-center justify-center group-hover:bg-primary transition-colors">
                  <Phone className="w-3.5 h-3.5 text-primary group-hover:text-white transition-colors" />
                </div>
                <span className="text-sm text-white/60 group-hover:text-white transition-colors font-medium">+44 7768 928 645 <span className="text-white/30 text-xs">(UK)</span></span>
              </a>
              <a href="mailto:info@muziclub.com" className="flex items-center gap-3 group w-fit">
                ...
                <span className="text-sm text-white/60 group-hover:text-white transition-colors font-medium">info@muziclub.com</span>
              </a>
            </div>
          </div>
          ...
          <div className="lg:col-span-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {centres.map((c) => (
                <div
                  key={c.name}
                  className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4 hover:border-white/10 hover:bg-white/[0.04] transition-all"
                >
                  <p className="text-sm font-bold text-white mb-2">{c.name}</p>
                  <div className="flex items-start gap-2 mb-2">
                    <MapPin className="w-3.5 h-3.5 text-primary shrink-0 mt-0.5" />
                    <p className="text-xs text-white/45 leading-relaxed">{c.address}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <Phone className="w-3.5 h-3.5 text-primary shrink-0" />
                    <a href={`tel:${c.phone.replace(/\s/g, "")}`} className="text-xs text-white/45 hover:text-primary transition-colors">{c.phone}</a>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="grid grid-cols-2 gap-x-6 gap-y-2.5">
              {courseLinks.map((c) => (
                <Link
                  key={c.slug}
                  to={`/courses/${c.slug}`}
                  className="text-sm text-white/40 hover:text-primary transition-colors flex items-center gap-2 group"
                >
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-8">
              {[
                ...
                <a
                  ...
                  rel="noreferrer"
                  title={label}
                  className="w-9 h-9 rounded-xl border border-white/10 flex items-center justify-center text-white/40 hover:text-white hover:border-primary/40 hover:bg-primary/10 transition-all"
                >
            ...
            <div className="space-y-1.5">
              {["Trinity College London", "ABRSM", "RSL – Rockschool", "G.M.V. Mumbai"].map((cert) => (
                <p key={cert} className="text-xs text-white/35 flex items-center gap-2">
        ...
        <div className="py-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-5 text-xs text-white/25">
            ...
            <Link to="/about" className="hover:text-white/60 transition-colors">Refund Policy</Link>
          </div>
        </div>
