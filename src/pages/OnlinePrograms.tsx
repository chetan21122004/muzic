...
const allCoursesList = courseCategories.flatMap((cat) => cat.courses.map((c) => ({ ...c, category: cat.title })));
const tabs = ["All", "Vocals", "Instruments", "Percussion", "Certifications"];
const formatIcons: Record<string, string> = { Vocals: "🎤", Instruments: "🎸", Percussion: "🥁", Certifications: "🏆" };
      ...
      <section className="py-10 border-b border-border bg-background">
        <div className="container mx-auto px-4">
          <div className="grid sm:grid-cols-2 gap-5 max-w-3xl mx-auto">
            <div className="rounded-2xl p-6 border border-border bg-secondary">
              <div className="text-2xl mb-3">🎸</div>
              <h3 className="font-bold text-foreground mb-1">Hobby Classes</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">Customized based on your individual preferences — aimed at enabling skills to play specific songs or music styles you love.</p>
            </div>
            <div className="rounded-2xl p-6 border border-border bg-secondary">
              <div className="text-2xl mb-3">⭐</div>
              <h3 className="font-bold text-foreground mb-1">Grade Classes &amp; Certification</h3>
              <p className="text-sm text-muted-foreground leading-relaxed">Follow the structured learning content of internationally recognized exams — Trinity, ABRSM, RSL, and GMV.</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-12 bg-background">
        <div className="container mx-auto px-4">
          <div className="flex flex-wrap gap-2 mb-10 justify-center">
            {tabs.map((tab) => (
              <button key={tab} onClick={() => setActiveTab(tab)} className={`px-5 py-2 rounded-full text-sm font-semibold border transition-all ${activeTab === tab ? "bg-primary text-primary-foreground border-primary shadow-lg shadow-primary/20" : "text-muted-foreground border-border hover:border-primary hover:text-primary"}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-5">
            {filtered.map((course) => (
              <Link key={course.slug} to={`/courses/${course.slug}`} className="group border border-border rounded-2xl p-5 hover:border-primary/30 hover:-translate-y-0.5 hover:shadow-lg transition-all duration-200 bg-background">
